...
- Onboarding wizard: when no profile exists, the side panel opens with a setup flow instead of the bare analyze view — upload a resume (PDF or .txt, drag-drop or file picker), extract locally in the panel via `pdfjs-dist` (dynamic chunk + `?url` worker, line-aware `hasEOL` extraction), map through the heuristic parser into a reviewable `CareerProfile` seed, then confirm. The review screen is fully editable (name/contact, skill chips, experience + education cards with add/remove, current-role toggle, per-line highlights) and marks fields the resume didn't contain with dashed outlines; "Confirm & continue" saves via `saveProfile`, "Open full editor" jumps to options. Resume→profile mapping moved from `apps/cli/src/resume-mapper.ts` into `@veya/document-engine` (`resume-to-profile.ts`, `crypto.randomUUID()` so it runs in browser + Node) and is shared with `veya resume parse`; CLI + tests refactored. Document-engine now has a `./light` subpath export (text parser + mapper only, no `pdf-parse`/`pdf-lib`); `pdf-parse` and `pdf-lib` became dynamic imports so browser bundles don't inline them — sidepanel bundle 894KB→23KB, release zip 930KB→622KB. Added `pdfjs-dist@4` (v6 threw `r.destroy is not a function` in the extension page) and `canvas: false` in `pnpm-workspace.yaml` (pdf-parse's native build is never used; text extraction needs no canvas). Verified end-to-end headless with a real pdf-lib-generated resume PDF: wizard → review (name/email/phone/skills/experience/education extracted) → confirm → analyze view; zero console errors, no horizontal overflow at 400px.
