...
- @veya/website (`apps/website`): React 19 + Vite landing page redesigned to the "Career Timetable" world (impeccable concept-seed `2bad5a15`, candidate 6) — warm paper/ink monochrome (paper #f1eee6 / ink #17140e, no accent hue), Archivo display + Instrument Serif italic counterpoint + IBM Plex Mono data, hairline rules, near-square 2–3px corners, one authored motion moment (the live departure-board hero that ticks its clock and sweeps an ink scan line; respects `prefers-reduced-motion`). Sections: station ticker + masthead, hero board (form fields as train departures: 8 detected · 5 verified · 1 drafted), trust strip (no cloud account / no training / no résumé scraping / no tracking), route steps (STOP 01/02/03), features ledger (ruled 2-col grid), inverted ink privacy panel (/no-backend · /no-training · /no-guessing · /byok), open source, install, footer. Website overrides `@veya/shared` tokens site-wide (incl. monochrome `--veya-accent`) so the shared LogoMark renders in ink. Light-only (`color-scheme: light`). Verified headless: 0px horizontal overflow at 375/768/1440, fonts loaded, board renders, body contrast 15.85:1, no console errors. Recorded in `PRODUCT.md` + `DESIGN.md` + `.impeccable/design.json`.
