...
- @veya/website (`apps/website`): React 19 + Vite landing page redesigned to the "Career Timetable" world (impeccable concept-seed `2bad5a15`, candidate 6) — warm paper/ink monochrome (paper #f1eee6 / ink #17140e, no accent hue), Archivo display + Instrument Serif italic counterpoint + IBM Plex Mono data, hairline rules, near-square 2–3px corners, one authored motion moment (the live departure-board hero that ticks its clock and sweeps an ink scan line; respects `prefers-reduced-motion`). **Expansive layout (v2):** 1400px measure + full-bleed bands; hero = headline + demo-video slot (disabled play button, video to be produced via demo-video skill) with the full-width departure board as the station floor beneath; trust strip; route steps (STOP 01/02/03); features ledger (ruled 2-col grid); full-bleed inverted ink privacy band (/no-backend · /no-training · /no-guessing · /byok); full-bleed deep-paper closing band merging open source + install (split by a vertical hairline). 6 content sections. Website overrides `@veya/shared` tokens site-wide (incl. monochrome `--veya-accent`) so the shared LogoMark renders in ink. Light-only (`color-scheme: light`). Verified headless: 0px horizontal overflow at 375/768/1024/1440/1920, board stretches to the 1400px measure, fonts loaded (serif italic confirmed), privacy full-bleed, finale collapses to 1 col, detector clean, no console errors. Recorded in `PRODUCT.md` + `DESIGN.md` + `.impeccable/design.json`.
